...
def train_model(train_iter, min_count=5, threshold=100.0, sep='_'):
    ...
    vocab = filter_vocab(vocab, min_count)  # vocab 词和频率的map，词包括一元词和二元词
    print ("Filtered Vocab", len(vocab))
    all_sentences = []
    new_words = set()
    for line in train_iter:  # line : [w1,w2,w3...]
        if len(line) > 0:
            out_sentence = []
            pairs = pairwise(line)
            for pair in pairs:
                ...
                if all((pa, pb, pab)):
                    ...
                    score = 0.0
                if score > threshold:
                    next_pair = next(pairs)
                    next_pa = vocab.get(next_pair[0])
                    next_pb = vocab.get(next_pair[1])
                    next_pab = vocab.get(next_pair)
                    if all((next_pa, next_pb, next_pab)):
                        next_score = (vocab.get(next_pair) - min_count) / vocab.get(next_pair[0]) / vocab.get(
                            next_pair[1]) * train_words
                    else:
                        next_score = 0.0
                    if score >= next_score:  # 第一对比下一对分值高
                        out_sentence.append(sep.join(pair))
                        new_words.add(sep.join(pair))
                    # 严格地说，如果第一对比下一对分值低,那么下一对还要跟下下对比较，
                    #  这样不停的循环比较直到得到连续递增的一个最大值点，这里简化
                    else:
                        out_sentence.append(pair[0])
                        out_sentence.append(sep.join(next_pair))
                        new_words.add(sep.join(next_pair))
                        next(pairs)
                else:
                    out_sentence.append(pair[0])
            all_sentences.append(out_sentence)
    return all_sentences,new_words
...
def main():
    ...
    train_file = (line.split() for line in open(options.train_file))
    out = train_file
